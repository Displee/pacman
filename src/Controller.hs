...
{-move g@(Ghost gx gy gi gis gt posg@(Tile gtx gty tt) pl dg ngd vg Frighten tlx tly ct) xs= (Ghost gx gy gi gis gt posg  dg dg (Just (xs!! (random))) vg m tlx tly ct)-}
move g@(Ghost gx gy gi gis gt posg@(Tile gtx gty tt) pl dg ngd vg m tlx tly ct) (d:ds) | d == South  && gty <= tly  =    (Ghost gx gy gi gis gt posg  dg dg (Just South) vg m tlx tly ct)
                                                                                       | d == West && gtx >= tlx  =      (Ghost gx gy gi gis gt  posg  dg dg (Just West) vg m tlx tly ct)
...
{-randomer :: StdGen -> Int
randomer =  randomR (0,3) pureGen where
             pureGen = mkStdGen 3  -}
...
targetBlinky d x y g@(Ghost gx gy gi gis gt pg pl dg nd vg m ttx tty ct) | m == Chase = Ghost gx gy gi gis gt pg pl dg nd vg m x y ct
                                                                         | m == Scatter = Ghost gx gy gi gis gt pg pl dg nd vg m 28 1 ct
                                                                         | otherwise = g
...
targetClyde _ x y (Ghost gx gy gi gis gt (Tile tx ty tt) pl dg nd vg m ttx tty ct) | m == Chase  = proxof8tiles
                                                                                   | m == Scatter = (Ghost gx gy gi gis gt (Tile tx ty tt) pl dg nd vg m 1 36 ct)
                    ...
                    proxof8tiles   | ((((x+y) >= (tx+ty)) && (((x+y) - (tx+ty)) <= 8)) ||  (((tx+ty) >= (x+y)) && (((tx+ty) - (x+y)) <= 8))) = (Ghost gx gy gi gis gt (Tile tx ty tt) pl dg nd vg m ttx tty ct)
                                   | otherwise  =(Ghost gx gy gi gis gt (Tile tx ty tt)  pl dg nd vg m 1 36 ct)
...
targetInky d x y g@(Ghost gx gy gi gis gt pg pl dg nd vg m ttx tty ct)  | m == Chase = case d of
                                                                        ...
                                                                        | m == Scatter = (Ghost gx gy gi gis gt pg pl dg nd vg m 28 36 ct)
                                                                        | otherwise = g
